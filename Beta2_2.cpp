...
//     printf("Novo saldo da conta de origem (%d): %.2f\n", origem->numero, origem->saldo);
//     printf("Novo saldo da conta de destino (%d): %.2f\n", destino->numero, destino->saldo);

//     salvarContas();
// }

void transferencia() {
    char contaOrigem[TAM_ID], contaDestino[TAM_ID], id_origem[TAM_ID], id_destino[TAM_ID];
    float valor, saldo_origem, saldo_destino;
    char auxiliar[50], anterior[50];
    bool conta_encontrada = false;
    FILE *fp, *fp_temp;
    ...
    strcpy(contaOrigem, auxiliar);
    printf("Conta origem: %s", contaOrigem);
    ...
    strcpy(contaDestino, auxiliar);
    printf("Conta destino: %s", contaDestino);
    ...
    // Lendo e processando o arquivo original
    while (fscanf(fp, "%[^,], %[^,], %[^,], %f\n", id_origem, getchar(), getchar(), &saldo_origem) != EOF) {
        // Comparando os dados lidos com os dados fornecidos pelo usuário
        if (strcmp(id_origem, contaOrigem) == 0) {
            conta_encontrada = true;
    ...
    if (!conta_encontrada) {
        ...
        remove("temp.txt"); // Remove o arquivo original
        rename("temp.txt", "contas.txt"); // Renomeia o arquivo temporário para o nome original
    }
    ...
    printf("Transferência realizada com sucesso!\n");
    //printf("Novo saldo da conta de origem (%s): %.2f\n", origem->id, origem->saldo);
